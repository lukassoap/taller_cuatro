...
<div class="container">
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <a class="btn btn-primary" href="{{ url('/taller/create') }}">Nueva Solicitud</a>
    </div>
                    ...
                    @if(count($Solicitud) > 0)
                        @foreach($Solicitud as $solicitud)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ url('/taller/' . $solicitud->id . '/edit') }}" class="btn btn-sm btn-warning">Editar</a>
                                    <form action="{{ url('/taller/' . $solicitud->id) }}" method="POST" style="display:inline-block;">
                                        @csrf
                                        @method('DELETE')
